...
while ($i<=25){
	...
	$button_num = $i+25*$_SESSION['single']['fight'];

	//If it was pressed prepare to call fight function
	if (isset($_POST['fight'.$button_num])){

		$_SESSION['fight']['status']='on';
		$enemy='enemy'.$button_num;
		$enemy_img='enemy'.$button_num;
		$result=pg_fetch_row(pg_query("SELECT char_nick FROM characters_main WHERE char_name='".$enemy."';"));
		$enemy_nick=$result[0];
	}
...
if (isset($_POST['next'])){
	...
	$_SESSION['single']['fight']=$_SESSION['single']['fight']+1;
...
while ($i<=25){
	...
	echo '#fight'.$i.'{';
    echo 'background-image:url(/resources/images/monsters/enemy_'.$button_num.'.png)';
	echo '}';
...
?>
    <div id="main">
        <div class="header">
            <h1>Single player</h1>
            <h2>Select a level to fight the monster</h2>
        </div>
...
?>
    <div id="main">
        <div class="header">
			...
			<div class="title3">
			<?php
			echo '<img class="pure-img" src="/resources/images/enemies/'.$enemy_img.'.png" alt="HTML5 Icon" style="width:128px;height:128px;">' ?>
			</br>
			<?php echo $enemy_nick?>
			</div>
        </div>
...
<form class="pure-form" name ="select" method="post" >
...
if ($_SESSION['fight']['status']==='off'){
	...
	while ($i<=25){
		...
		echo '<button type="submit"  name="fight'.$button_num.'" id="fight'.$button_num.'" class="buttonimg"> Level '.$button_num.' </button>';
		echo '&nbsp&nbsp&nbsp';
		//Every 5 loops change row
		$line_breaker=$i % 5;
	...
	//Show the buttons next and previous

	//Previous
	echo '<div class="bottom1">';
	if ($_SESSION['single']['fight']>=1){

		echo '<button type="submit"  name="previous" id="previous" class="pure-button pure-button-primary"> Previous </button>';

	}
		echo '</div>';
	//Next
		echo '<div class="bottom2">';
	if ($_SESSION['single']['fight']<4){

		echo '<button type="submit"  name="next" id="next" class="pure-button pure-button-primary"> Next </button>';

	}
		echo '</div>';
}else{
	combat_func($_SESSION['character']['char_name'],'enemy_'.$enemy);
}

...
//get all the matches of the character
$query="SELECT * FROM replaytable WHERE player1='".$_SESSION['character']['char_nick']."' or player2='".$_SESSION['character']['char_nick']."' ORDER BY time_created desc;";
$data=pg_query($query);
...
<div id="layout">
    ...
    <div id="menu">
        ...
        <li class="pure-menu-item pure-menu-selected"><a href="history.php" class ="pure-menu-link">History</a></li>
    ...
    <div id="main">
        <div class="header">
		<h1> Replays of your character </h1>
		</div>
		<div class="content">
		<?php
...
if ($row==0){
 echo 'no replays found';
	...
	$result=pg_fetch_assoc($data);

	if ($result['winner']===$_SESSION['character']['char_nick']){
		//Player 1
		echo '<div>  '.$result['player1'].' vs  '.$result['player2'].' </div>';
		//Player 2
		echo '<div><a ><button>Play</button></a> </div>';
	}else{
		//Player 1
		echo '<div>  '.$result['player1'].' vs  '.$result['player2'].' </div>';
		//Player 2
	...
	while (isset($result[$i]['winner'])){

		if ($result[$i]['winner']===$_SESSION['character']['char_nick']){
			//Player 1
			echo '<div>  '.$result[$i]['player1'].' vs  '.$result[$i]['player2'].' </div>';
			//Player 2
			echo '<div><a ><button>Play</button></a> </div>';
		}else{
			//Player 1
